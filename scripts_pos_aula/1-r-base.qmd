...
# objeto do tipo lógico (verdadeiro/falso) (TRUE ou FALSE)
estuda <- FALSE
estuda
# Verdadeiro -> 1
# Falso -> 0
...
1.  Tente adivinhar o tipo dos objetos abaixo: numérico, texto, lógico.
...
```{r}
class(altura) # character - texto
class(gosta_brocolis) # logical - lógico
class(idade) # numeric - double/dbl
```
...
primeiro_semestre[1] # em outras linguagens, começa no 0 
...
```{r}
# Coluna de uma tabela pode ser acessada com $
iris$Sepal.Length
...
Lembrete: olhar a documentação, na aba help, ou rodando `?funcao` ou `??funcao`. 
...
vetor_com_na == 1
vetor_com_na == NA # isso não funciona do jeito que imaginamos
```
...
library(remotes)
install_github("cienciadedatos/dados")
```
...
```{r}
tabela <- geobr::list_geobr()

sp <- geobr::read_state("SP")
